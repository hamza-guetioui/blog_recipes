...
const RECIPES_QUERY = `*[_type == "recipe" && trending == true] | order(_createdAt desc) {
    _id ,
    name,
    title,
    slug,
...
const Header = () => {
  return (
    <div className="flex justify-between items-center gap-4 px-4">
      <h1 className="font-bold text-3xl min-w-fit">The Newest Recipes</h1>
      <div className="w-full h-2 bg-gray-200 rounded-full border-3 border-dashed border-black"></div>

      <Link href={"/recipes"} className="font-semibold text-xl">
        all
      </Link>
    </div>
  );
